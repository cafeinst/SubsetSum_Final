...
corollary no_polytime_on_all_inputs:
  shows "\<not> (\<exists>(c::real)>0. \<exists>(d::nat).
            \<forall>as s. steps M (enc as s kk) \<le> nat \<lceil> c * real (length as) ^ d \<rceil>)"
proof
  assume "\<exists>(c::real)>0. \<exists>d. \<forall>as s. steps M (enc as s kk) \<le> nat \<lceil> c * real (length as) ^ d \<rceil>"
  then obtain c d where cpos: "c > 0"
    and UB: "\<forall>as s. steps M (enc as s kk) \<le> nat \<lceil> c * real (length as) ^ d \<rceil>" by blast
